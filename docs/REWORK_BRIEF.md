...
| 6.8b | D11 fill read-back (follow-up) | Replace CP target-price approximation in `_mark_order_filled` with real `fills-history` reads for Blofin (HL returns fallback — no fills endpoint). Entry matches cleanly by `orderId`; a triggered TP/SL conditional executes under a *new* execution `orderId` ≠ stored `tpslId`, so TP/SL matching is designed against a real captured triggered-conditional fill from the demo (D10/D11: validate, don't guess) — do during 6.10 soak. |
