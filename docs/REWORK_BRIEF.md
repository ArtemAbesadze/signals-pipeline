...
| 6.3 | `src/exchange/blofin.py` — hand-rolled HTTP client ✅ (2026-06-04) | BlofinClient: verified `sign()`, `_request`/`_checked`, reads (balance/positions[signed]/orders/mids/asset_meta/fills — D11) + writes (place/cancel/set_leverage/close_position). 25 tests vs real demo shapes. Standalone; wired into `build_exchange_client` at 6.5 once the position manager exists. |
| 6.4 | `src/exchange/blofin_order_builder.py` ✅ (2026-06-05) | Contract-value sizing (`USD/(price*contractValue)`), lotSize floor + minSize check, tickSize rounding (Bug #18 obviated). CP signal → 5 orders: entry (`/trade/order`) + SL + 3 TPs (`/trade/order-tpsl`, partial sizes — demo-confirmed). 19 tests incl. BTC contract-value sentinel. |
| 6.5 | `src/exchange/blofin_position_manager.py` ✅ (2026-06-05) | BlofinPositionManager mirrors PositionManager's contract: submit (entry=`/trade/order`, SL+3TP=`order-tpsl`, ids in `oid` TEXT, cancel routes by type) / cancel (`cancel-order` vs `cancel-tpsl`, 102068 benign) / close (`close-position` true market, verify before CLOSED) / move-SL / 3-source sync. Client gained place_tpsl/cancel_tpsl/get_open_tpsl_orders. 21 tests. **D11 fill read-back wires at the pipeline-dispatch step (next).** Not yet wired into orchestrator. |
...
| 6.8 | Orchestrator/pipeline dispatch wiring ✅ (2026-06-05) | `src/exchange/adapter.py` — `ExchangeAdapter` (`HyperliquidAdapter` / `BlofinAdapter`) encapsulates the only divergences (order builder, position manager, `entry.sz` vs `entry.size`, balance field, symbol map, `$10` floor). `Pipeline` builds its adapter from `config.exchange.exchange` (constructor unchanged → HL tests untouched); `build_exchange_client`'s Blofin branch now constructs `BlofinClient` (`mainnet→production`, else `demo`); orchestrator sync/kill use `build_position_manager`. **HL path byte-identical.** 20 new tests (`test_exchange_adapter.py` + dispatch + Blofin-client-build). **D11 fill read-back deferred to 6.8b** (below). |
| 6.8b | D11 fill-price read-back ✅ (2026-06-05) | `_mark_order_filled` now persists the exchange's REAL `averagePrice` for Blofin (HL keeps the CP estimate — no endpoint). Demo soak revealed the clean join: `orders-history` rows carry our `potion_{trade_id}_{type}` as `clientOrderId` (entries) / `algoClientOrderId` (triggered TP/SL — execution `orderId` ≠ `tpslId`, `algoId`=tpslId). `BlofinClient.get_orders_history` + `adapter.real_fill_price` (matches by that label, falls back to CP on no-match/failure). New `orders.fill_price_source` column ('exchange'\|'cp_estimate') — D11 "never silently mix". 11 tests. |
| 6.8b-2 | D11 realized-PnL read-back ✅ (2026-06-05) | Close handlers (`_handle_all_tp_hit` / `_handle_stop_hit`) now use the exchange's REAL realized PnL when available instead of CP's leveraged %. `adapter.realized_pnl` sums `orders-history.pnl` over the trade's `potion_{trade_id}_*` orders; `Pipeline._close_with_realized_pnl` backs out `pnl_pct = real_usd / collateral` and applies the real USD to the port, tagging new `trades.pnl_source` ('exchange'\|'cp_estimate'). HL unchanged (no endpoint → CP). Fixes the soak gap where DB recorded CP's signal-leverage % while actual leverage was capped. 6 tests. |
| 6.8d | BE/SL-move safety ✅ (2026-06-05) | `move_stop_loss` no longer leaves a Blofin position unprotected: pre-validates the new SL price vs current market (skip + keep old SL if it'd be rejected) and re-instates the original SL (CRITICAL alert) if the new placement fails after the old was canceled. 3 tests. |
| 6.9 | `/register` flow — exchange choice ✅ (2026-06-05) | New users pick HL or Blofin first; the credential sub-flow branches (HL: address/wallet/secret · Blofin: key/secret/passphrase, alphanumeric, deleted on receipt, stored in neutral keys with empty `api_wallet`). Shared network step labels Demo/Live for Blofin (callbacks stay testnet/mainnet → `build_exchange_client` maps demo/production). Completion validates per exchange (HL `get_account_state`, Blofin `get_balance`) and persists `exchange`+`passphrase`. 14 tests. **Pending (D11 menus):** Telegram balance/positions/PnL screens still read HL-shaped data — Blofin menu read-back is part of the 6.8b/6.11 D11 work. |
| 6.10 | Test driver wiring ✅ (2026-06-05) | `scripts/test_driver.py` dispatches coin-pool + prices on the target exchange (`exchange:` config key, else config.yaml) — Blofin draws from the public `/market/instruments` + `/market/tickers` (credential-free) via `build_market_pool` / `filter_to_blofin_universe`. Orphan cancel (`cancel_orphan_orders`) routes per-user on `creds["exchange"]` (Blofin entry→cancel-order, SL/TP→cancel-tpsl); `_list_test_orders_with_oid` now returns `(user, oid_str, coin, order_type)`. `--top-up-demo` (experimental — `BlofinClient.demo_apply_money`, body shape probed against the live demo). +12 tests. **6.11 soak is where the demo body shape + 6.8b triggered-fill shape get confirmed.** |
| 6.11 | Soak on Blofin demo ✅ (2026-06-05 — 3 runs, green confirmation done) | Three demo soaks (3-trade smoke + two 5-scenario). **Validated:** full order set placement (entry+SL+3TP, real Blofin ids, 0 rejections), TP/SL/BE/stop/all-TP handling, **D10 refuse-promote** (entry-not-filled) + **D10 close-on-cancel** (entry filled → cancel → real position market-closed, fired on 2 trades in the green run), **6.8b real `exchange` fill read-back live** (multiple entries, e.g. RENDER 1.728 / WIF 0.159 / ADA 0.1637 read from orders-history vs CP target). **Bugs found+fixed (6.8c):** set_leverage abort, terminal-state resurrection, test-driver coin reuse — all confirmed fixed in the green run (0 resurrections, 0 submission_failed, distinct coins). **Open finding (not yet fixed):** `move_stop_loss` cancels the old SL before placing the new one, so a rejected BE-SL move (e.g. price crossed back past entry) leaves the position momentarily unprotected — surfaced via synthetic TP1 + adverse real price on both LONG/SHORT; recommend alert-on-failure or place-then-cancel. **Note:** blofin_demo risk limits left raised (daily-loss/exposure) so synthetic leveraged losses don't trip the breaker mid-soak. |
| 6.8c | Soak-hardening fixes ✅ (2026-06-05) | See 6.11. `BlofinPositionManager.submit_trade` leverage-resilient; `Pipeline._is_terminal` guard on tp/stop/all-TP handlers; `test_driver.select_coins` distinct coins. |
| 6.12 | Per-user migration 🟡 (code done 2026-06-05; cutover pending) | In-bot exchange switching, HL stays the fallback. **Decisions:** leverage uncapped = global policy (`max_leverage=0` = follow CP, clamp to instrument max, `3bf3169`); creds persist per (exchange, network); switching re-enters creds only for a new combo; main menu shows exchange + Demo/Mainnet. **Shipped:** step 1 per-(exchange,network) credential storage + active pointer (`f7e8e54`, live DB migrated); step 2 D11 menus — badge + exchange-aware balance (`092d25b`); step 3 switch flow Config → 🔁 Switch Exchange (`1e1ec90`) — hard-block on open trades, saved-combo reuse, atomic commit + reactivate, mainnet-token gate. +21 tests. **Remaining:** step 4 = Artem migrates himself to Blofin demo via the flow (operational), then friends; production cutover deferred (testnet/demo only for now); the BE-SL move hardening (6.11 open finding) still open. |
