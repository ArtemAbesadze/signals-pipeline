...
| 6.8b | D11 fill-price read-back ✅ (2026-06-05) | `_mark_order_filled` now persists the exchange's REAL `averagePrice` for Blofin (HL keeps the CP estimate — no endpoint). Demo soak revealed the clean join: `orders-history` rows carry our `potion_{trade_id}_{type}` as `clientOrderId` (entries) / `algoClientOrderId` (triggered TP/SL — execution `orderId` ≠ `tpslId`, `algoId`=tpslId). `BlofinClient.get_orders_history` + `adapter.real_fill_price` (matches by that label, falls back to CP on no-match/failure). New `orders.fill_price_source` column ('exchange'\|'cp_estimate') — D11 "never silently mix". 11 tests. **Remaining 6.8b-2:** realized-PnL read-back (`orders-history.pnl` is real per-order; needs careful pct conversion before replacing CP's `pnl_pct`). |
