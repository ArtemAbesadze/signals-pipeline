...
| 6.11 | Soak on Blofin demo 🟡 (2026-06-05 — 2 runs done, 3 bugs found+fixed) | Two demo soaks (3-trade smoke + 5-scenario). **Validated:** full order set placement (entry+SL+3TP, real Blofin ids, 0 rejections), TP/SL/BE/stop/all-TP handling, **D10 refuse-promote** (entry-not-filled), **D10 cancel routing**, **6.8b real `exchange` fill read-back live** (POL entry 0.07788 vs CP 0.07779). **Bugs found + fixed (6.8c):** (1) `set_leverage` aborted a trade when the instrument had pending orders → now logs + proceeds at current leverage; (2) tp/stop/all-TP handlers **resurrected** CANCELED/CLOSED trades with phantom PnL → terminal-state guard added; (3) test driver reused coins (Blofin same-instId collision) → `select_coins` samples distinct. +6 tests (944 total). **Remaining:** a clean green soak after the fixes; the close-on-cancel D10 branch still needs a guaranteed-filled entry to exercise. |
